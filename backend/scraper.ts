...
const REQUEST_TIMEOUT = 30000; // 30 seconds - some devices are slow
...
function looksLikeStatsPage(html: string) {
  ...
  // If there's a password input form, it's likely a login page
  const hasPasswordField = $('input[type="password"]').length > 0;
  if (hasPasswordField) {
    return false; // It's a login page, not a stats page
  }
  
  // Check for ACTUAL sales/income data indicators (not just generic keywords)
  const hasSalesData = 
    ...
    (text.includes('revenue') && text.includes('php')) ||
    (text.includes('income') && text.includes('amount'));
...
async function attemptFormLogin(client: ReturnType<typeof wrapper>, baseUrl: string, device: any) {
  ...
  for (const loginPageUrl of loginPageUrls) {
    try {
      ...
      const followUp = await client.get(`${baseUrl}/admin/system`, {
        timeout: REQUEST_TIMEOUT,
        validateStatus: () => true,
      });
      const followUpHtml =
      ...
      if (followUp.status === 302 || looksLikeLoginPage(followUpHtml)) {
        return {
          ...
          error: `Login rejected for username "${device.username}" at ${baseUrl}`,
        };
      }
    } catch (err: any) {
...
export async function scrapeDevice(deviceId: number) {
  ...
  try {
    ...
    if (!statsPage) {
      ...
      if (!statsPage) {
        throw new Error('Logged in but could not load the admin stats page');
      }
    }
    ...
    db.prepare('INSERT INTO scrape_logs (device_id, status, message) VALUES (?, ?, ?)')
      .run(device.id, 'success', `Scraped daily income: PHP ${incomeAmount}`);
